...
{ 
	...
	if ((vehicle player) == _x && ((getPos _x) select 2 < 1) && !(player call tz_util_isDonator) && !(player call tz_util_isAdmin)) then {

		{
			// Let player know this is members area and they are being removed
			hintC "This is a Donor only area!";
			// Teleport them
	...
	if (player == _x && !(_x call tz_util_isDonator) && !(player call tz_util_isAdmin)) then {
		// Let player know this is members area and they are being removed
		hintC "This is a Donor only area!";
		// Teleport them
		_x setPos (getMarkerPos "respawn_west");
