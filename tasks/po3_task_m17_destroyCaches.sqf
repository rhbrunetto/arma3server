private["_location","_position","_locaname","_marker"];
// =========================================================================================================
...
_b = _b min 15;
	...
	for "_i" from 1 to /*round(_b*0.75)*/ 2 do {
	...
	_b3_cache_markers = [];
	{
		private ["_name", "_marker", "_lx", "_ly"];
	
		_name = format["b3_cache_marker_%1", _b3_cache_index];

		_lx = (((getPos _x) select 0) + (round(random 50) - 25));
		_ly = (((getPos _x) select 1) + (round(random 50) - 25));

		_marker = createMarker [_name, [_lx, _ly]];
		_name setMarkerPos [_lx, _ly];
		_name setMarkerShape "ELLIPSE";
		_name setMarkerBrush "DiagGrid";
		_name setMarkerColor "ColorRed";
		_name setMarkerSize [50, 50];

		_b3_cache_markers set [count _b3_cache_markers, _marker];

		_b3_cache_index = _b3_cache_index + 1;
	} forEach _caches;
...
// =========================================================================================================
	...
	waitUntil { count([_position,250,(PO3_side_3 select 0)] call PO3_fnc_getNearbyUnits) <= 5 };
